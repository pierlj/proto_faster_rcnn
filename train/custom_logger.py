    ...
    def new_training(self, config, dataset=None):
        if self.notify:
            if dataset is not None:
                text = '\U00002699 *New training started on {}*\n_Config:_ \n{}'.format(dataset.name, config)
            else:
                text = '\U00002699 *New training started*\n_Config:_ {}'.format(config)
            self.bot.sendMessage(self.chat_id, text=text, parse_mode='Markdown')
    
    def training_end(self, map_value):
        if self.notify:
            text = '\U00002705 *Training done*\nFinal mAP value: {:.3f}'.format(map_value)
            self.bot.sendMessage(self.chat_id, text=text, parse_mode='Markdown')
        ...
        if self.notify:
            ...
            if update != []:
                ...
                if update[-1]['message']['text'] == '/stop':
                    ...
                    self.bot.sendMessage(self.chat_id, text, parse_mode="Markdown")
