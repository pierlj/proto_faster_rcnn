...
class DetectionMAP:
    ...
    def evaluate(self, pred_bb, pred_classes, pred_conf, gt_bb, gt_classes):
        ...
        :return:
        """
        # print(pred_bb.shape, pred_classes.shape, pred_conf.shape, gt_bb.shape, gt_classes.shape)
        if pred_bb.ndim == 1:
    ...
    def evaluate_(IoUmask, accumulators, pred_classes, pred_conf, gt_classes, confidence_threshold):
        pred_classes = pred_classes.astype(np.int) - 1 # label 0 was assigned to bg
        gt_classes = gt_classes.astype(np.int) - 1 # label 0 was assigned to bg
    ...
    def plot_pr(self, ax, class_name, precisions, recalls, average_precision):
        ...
        ax.set_title('{0:} : AUC={1:0.2f}'.format(
    ...
    def plot(self, interpolated=True, class_names=None):
        ...
        for i, ax in enumerate(axes.flat):
            ...
            class_name = class_names[i] if class_names else "Class {}".format(
                i)
            self.plot_pr(ax, class_name, precisions,
                         recalls, average_precision)
            mean_average_precision.append(average_precision)

        plt.suptitle("Mean average precision : {:0.2f}".format(
            sum(mean_average_precision)/len(mean_average_precision)))
        fig.tight_layout()
    
    def compute_ap_classe(self, interpolated=True, class_names=None):
        """
        Plot all pr-curves for each classes
        :param interpolated: will compute the interpolated curve
        :return:
        """
        self.map_dict = {}
        self.mean_average_precision_classes = []
        # TODO: data structure not optimal for this operation...
        for i in range(self.n_class):
            ...
            self.mean_average_precision_classes.append(average_precision)
            self.map_dict[class_name] = average_precision


        self.average_map = sum(
            self.mean_average_precision_classes)/ \
                len(self.mean_average_precision_classes)
    
    def get_AP(self, return_map=False, verbose=False):
        if verbose:
            print('-------------------------------')
            print('VOC class mAP')
            print('IoU threshold: {}'.format(self.overlap_threshold))
            print('-------------------------------')
            for key, value in self.map_dict.items():
                print('{}: {:.2f}'.format(key, value))
            print('-------------------------------')
            print('Average over classes: {}'.format(self.average_map))
