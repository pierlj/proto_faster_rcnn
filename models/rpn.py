...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.anchors = None # anchors buffer to prevent mem copy
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                # print(true_match.min(), true_match.max(),
                #       true_match.mean(), true_match.std())
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        final_boxes = []
        final_scores = []
        with profiler.record_function("First where"):
            for boxes, scores, lvl, img_shape in zip(proposals, objectness, levels, image_shapes):
                with profiler.record_function("Clip"):
                    boxes = box_ops.clip_boxes_to_image(boxes, img_shape)
                with profiler.record_function("Small boxes"):
                ...
                with profiler.record_function("Append"):
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        # self.output_crop(
        #     sampled_pos_inds[random.randint(0, len(sampled_pos_inds)-1)])
        sampled_inds = torch.cat([sampled_pos_inds, sampled_neg_inds], dim=0)

        objectness = objectness.flatten()
        # print(sampled_pos_inds.numel())
        ...
        # multiply loss by focal weights
        gamma = 2.0
        probas = torch.clamp(F.sigmoid(objectness[sampled_inds]), 1e-4, 1.0 - 1e-4)
        probas_gt = torch.where(labels[sampled_inds].bool(), probas, 1 - probas)
        focal_weights = torch.pow(1 - probas_gt, gamma)

        focal_loss = torch.mean(focal_weights * objectness_loss)

        # return objectness_loss, box_loss
        return focal_loss, box_loss

    def output_crop(self, idx):
        # number of proposal per image
        N = int(self.proposals_copy.numel() / 4 /
                len(self.images_copy.image_sizes))
        ...
        x2 = min(x2, 384)
        y2 = min(y2, 384)
        cv2.imwrite('test.jpg', self.images_copy.tensors[0, :, x1:x2, y1:y2].permute(
            1, 2, 0).cpu().numpy()*255)
    ...
    def forward(self,
        ...
        with profiler.record_function("RPN head"):
            objectness, pred_bbox_deltas = self.head(features)

        with profiler.record_function("RPN Anchors generation"):
        ...
        # the proposals
        with profiler.record_function("RPN boxes decoding"):
        ...
        with profiler.record_function("RPN boxes filtering"):
