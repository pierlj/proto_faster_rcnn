...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # print(labels.shape, class_logits.shape)
    # multiply loss by focal weights
    gamma = 2.0
    probas = torch.clamp(
        F.softmax(class_logits, dim=-1), 1e-4, 1.0 - 1e-4)
    probas_gt = torch.gather(probas, -1, labels.view(-1,1))
    focal_weights = torch.pow(1 - probas_gt, gamma)

    focal_loss = torch.mean(focal_weights * classification_loss)
...
def _onnx_paste_mask_in_image(mask, box, im_h, im_w):
    ...
    # Resize mask
    mask = F.interpolate(mask, size=(int(h), int(w)),
...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self,
        ...
        with profiler.record_function("ROI HEAD boxe decoding"):
    ...
    def forward(self,
        ...
        with profiler.record_function("ROI HEAD RoI Align"):
            box_features = self.box_roi_pool(features, proposals, image_shapes)

        with profiler.record_function("ROI HEAD box head"):
            box_features = self.box_head(box_features)

        with profiler.record_function("ROI HEAD boxe prediction"):
        ...
        if self.training:
            ...
            with profiler.record_function("ROI HEAD post processing"):
